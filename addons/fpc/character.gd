...
@export var UPWARD : String
@export var DOWNWARD : String
...
func check_controls(): # If you add a control, you might want to add a check for it here.
	...
	if !InputMap.has_action(UPWARD):
		push_error("No control mapped for move backward. Please add an input map control. Disabling movement.")
		immobile = true
	if !InputMap.has_action(DOWNWARD):
		push_error("No control mapped for move backward. Please add an input map control. Disabling movement.")
		immobile = true
...
func handle_state(moving):
	if sprint_enabled:
		if sprint_mode == 0:
			if Input.is_action_just_pressed("SPRINT") and state != "crouching":
				if moving:
					if state != "sprinting":
						enter_sprint_state()
				else:
