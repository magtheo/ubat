...
func generate_chunk():
	...
	for i in range(data_tool.get_vertex_count()):
		...
		vertex.y = noise.get_noise_2d(vertex.x + x, vertex.z + z) * 80
		#vertex.y = 
	...
	#for s in range(array_plane.get_surface_count()):
		#array_plane.clear_surfaces(s) # ikke nødvendig i føge llama3
	...
	mat_mesh.albedo_color = Color.NAVAJO_WHITE
	
	mesh_instance = MeshInstance3D.new()
	mesh_instance.mesh = surface_tool.commit()
	mesh_instance.create_trimesh_collision()
	mesh_instance.mesh.surface_set_material(0, mat_mesh)
