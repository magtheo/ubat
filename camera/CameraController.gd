...
func _input(event):
	if event is InputEventMouseMotion:
		...
		var target_yaw = clamp(rotation_helper.rotation.y + mouse_motion.x, -180, 180)
		rotation_helper.rotate_y(deg_to_rad(target_yaw))

		# Keep the camera's up direction aligned with the world's up direction
		var target_pitch = deg_to_rad(fmod(rotation_helper.rotation.x + mouse_motion.y, PI))
		...
		rotation_helper.rotate_x(deg_to_rad(target_pitch))
